...
// to be implemented by upsampling module
float spectrum_xyz_to_p(const float lambda, const float *xyz);
float spectrum_rgb_to_p(const float lambda, const float *rgb);
void spectrum_print_info(FILE *fd);
...
static inline float spectrum_eta_from_abbe(const float n_d, const float V_d, const float lambda)
{ 
  float A, B;
  spectrum_cauchy_from_abbe(n_d, V_d, &A, &B);

  // convert from micrometers to nanometers
  return A + B/(lambda*lambda) * 1e6f;
}

static inline float spectrum_rgb_to_reflectance(const float lambda, const float *rgb)
{
  // incredibly stupid gamut mapping to the gamut of valid reflectances:
  // magic number is such that an equal-energy spectrum created from xyz = (1,1,1) is a constant spectrum with unit height (==1).
  return fminf(1.0f, 106.86535f * spectrum_rgb_to_p(lambda, rgb));
}
...
static inline void spectrum_xyz(const float lambda, float *xyz)
{
  ...
  // xyz[0] = spectrum_xyz_lut[3*i+0];
  // xyz[1] = spectrum_xyz_lut[3*i+1];
  // xyz[2] = spectrum_xyz_lut[3*i+2];
}
...
{
  if(pdf) *pdf = 1.0f/(spectrum_sample_max - spectrum_sample_min);
  return spectrum_sample_min + (spectrum_sample_max - spectrum_sample_min)*rand;
}

static inline float spectrum_lambda_pdf(const float lambda)
{
  return 1.0f/(spectrum_sample_max - spectrum_sample_min);
}
