...
namespace Tungsten
{
                           ...
                           const Vec3pf *bounds, float tMin, float tMax, float radius, Vec3f &beamEstimate)
    {
        ...
        if (intersectBeam1D(beam, ray, bounds, tMin, tMax, radius, invSinTheta, t))
        {
            ...
            beamEstimate += medium->sigmaT(hitPoint) * invSinTheta / (2.0f * radius) * medium->phaseFunction(hitPoint)->eval(beam.dir, -ray.dir()) * medium->transmittance(sampler, mediumQuery, true, false) * beam.power;
                            ...
                            const TraceableScene *scene, float tMin, float tMax, Vec3f &beamEstimate)
    {
        ...
        if (intersectPlane0D(ray, tMin, tMax, p.p0, p.p1, p.p3, invDet, t, uv))
        {
            ...
            if (!scene->occluded(shadowRay))
            {
                Ray mediumQuery(ray);
                mediumQuery.setFarT(t);
                beamEstimate += sqr(medium->sigmaT(hitPoint)) * std::abs(invDet) * medium->phaseFunction(hitPoint)->eval(p.d1, -ray.dir()) * medium->transmittance(sampler, mediumQuery, true, false) * p.power;
            ...
            if (medium)
            {
                if (bounce > 1 || !useFrustumGrid)
                {
                    ...
                    auto pointContribution = [&](const VolumePhoton &p, float t, float distSq)
                    {
                        ...
                        Ray mediumQuery(ray);
                        mediumQuery.setFarT(t);
                        estimate += (3.0f * INV_PI * sqr(1.0f - distSq / p.radiusSq)) / p.radiusSq * medium->phaseFunction(p.pos)->eval(p.dir, -ray.dir()) * medium->transmittance(sampler, mediumQuery, true, false) * p.power;
                    };
