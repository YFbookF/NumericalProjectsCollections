    ...
    int eyeray)           // eye ray or not (interpolate voxels?)
{
  ...
  for (int k = 0; k < 3; k++)
    t[k] = pos[k] + tmin * dir[k]; // world space entry point
  for (int k = 0; k < 3; k++)
    p[k] = (int)fmaxf(0.0f, fminf(s->grid_size[k] - 1e-5f, (t[k] - aabb[k]) / cellsize[k]));
  for (int k = 0; k < 3; k++)
    p[k] >>= s->grid_shift; // now in supervoxel coordinates
  ...
  while (1)
  {
    ...
    t[mind] += delta[mind];
    p[mind] += step[mind];
    if (p[mind] >= s->grid_coarse_size[mind] || p[mind] < 0)
      return FLT_MAX;
  }
}
    ...
    int eyeray)           // eye ray or not (interpolate voxels?)
{
  ...
  while (1)
  {
    ...
    if (mu_c > 0.0)
    {
      ...
      if (transmittance < VOL_MIN_TRANSMITTANCE)
        return 0.0f;
    }
