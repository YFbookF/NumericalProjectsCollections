...
Camera::Camera()
{
	Position = Vector3f(5.0f, 4.0f,0.0f);
	Front = Position - Vector3f(0.0f, 0.0f, 0.0f);
	Up = Vector3f(0.0f, 1.0f, 0.0f);
	Zoom = 45.0f;
}

Camera::Camera(float px, float py, float pz, float lx, float ly, float lz)
{
	Position = Vector3f(px, py, pz);
	Front = Position - Vector3f(lx,ly,lz);
	Up = Vector3f(0.0f, 1.0f, 0.0f);
	Zoom = 45.0f;
}
...
void Camera::SetPerspectiveMatrix2(float left,float right,float top,float bottom,float near,float far)
{
...
void Camera::UpdateUpAndRightVector()
{
	Front =  Vector3f(0, 0, 0) - Position ;
	Front = Front.normalized(); // 向前向量
	RightVector = -Cross(Front, Up).normalized(); // 右向量
	UpVector = -Cross(RightVector, Front); // 上向量
}
...
{
	Front = Position - Vector3f(0.0, 0.0, 0.0);
	Vector3f f = Front.normalized(); // 向前向量
