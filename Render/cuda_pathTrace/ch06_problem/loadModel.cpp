...
void loadObj(const std::string filename,int & obj_triangle_num)
{
	...
	obj_triangle_num = triangle_num;
	scene_vertices_pos = (Vertex*)malloc(vertices_num * sizeof(Vertex));
	scene_triangles = (Triangle*)malloc(triangle_num * sizeof(Triangle));
	...
	while (!in2.getline(buffer, 255).eof())
	{
		...
		if (buffer[0] == 'v' && (buffer[1] == ' ' || buffer[1] == 32)) {
			if (sscanf(buffer, "v %f %f %f", &f1, &f2, &f3) == 3) {
				scene_vertices_pos[vertex_read_idx].x = f1;
				scene_vertices_pos[vertex_read_idx].y = f2;
				scene_vertices_pos[vertex_read_idx].z = f3;
		...
		else if (buffer[0] == 'f' && (buffer[1] == ' ' || buffer[1] == 32))
		{
			...
			/*
			int nt = sscanf(buffer, "f %d//%d %d//%d %d//%d", &f4, &junk0,
				&f5, &junk1, &f6, &junk2);
			if (nt != 6) {
				std::cout << "ERROR: I don't know the format of that FaceMtl" << "\n";
				exit(-1);
			...
			*/
			scene_triangles[triangle_read_idx]._idx1 = f4;
			scene_triangles[triangle_read_idx]._idx2 = f5;
			scene_triangles[triangle_read_idx]._idx3 = f6;
			//printf("%d %d %d \n", scene_triangles[triangle_read_idx]._idx1, scene_triangles[triangle_read_idx]._idx2, scene_triangles[triangle_read_idx]._idx3);
			triangle_read_idx += 1;
		}
		lineno += 1;
	}
