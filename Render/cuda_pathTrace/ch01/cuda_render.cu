...
void loadObj(const std::string filename, TriangleMesh& mesh)
{
	...
	while (!in.getline(buffer, 255).eof())
	{
		...
		{
			TriangleFace f;
			float junk0, junk1, junk2, junk3, junk4, junk5;
			int nt = sscanf(buffer, "f %d/%d/%d %d/%d/%d %d/%d/%d", &f.v[0],&junk0,&junk1, &f.v[1],&junk2,&junk3, &f.v[2],&junk4,&junk5);
			if (nt != 9) {
