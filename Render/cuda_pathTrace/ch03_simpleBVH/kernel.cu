...
__device__ bool RayBoxIntersection_bool(int boxIdx, const Vector3f& rayOri, const Vector3f& rayDir)
{
...
void disp(void)
{
	...
	render_kernel << < grid, block >> > (dptr, accumulatebuffer, total_number_of_scene_triangles, frames, WangHash(frames), scene_aabbox_max, scene_aabbox_min);  // launches CUDA render kernel from the host
...
void initCUDAmemoryTriMesh()
{
	...
	for (unsigned int i = 0; i < mesh1.faces.size(); i++)
	{
		...
		cuda_scene_triangles.push_back(make_float4(v0.x, v0.y, v0.z, 0));
		cuda_scene_triangles.push_back(make_float4(v1.x - v0.x, v1.y - v0.y, v1.z - v0.z, 1));
		cuda_scene_triangles.push_back(make_float4(v2.x - v0.x, v2.y - v0.y, v2.z - v0.z, 0));
	}
	...
	*/
	//std::cout << "total number of scene_triangles check:" << mesh1.faces.size() + mesh2.faces.size() << " == " << cuda_scene_triangles.size() / 3 << std::endl;
	
	// calculate total number of scene_triangles in the scene
