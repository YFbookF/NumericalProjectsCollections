...
while(time < timeFinal):
    ...
    for ie in range(element_num):
        ...
        area = 0.5
        dH0dx = np.dot(dP0dx,minv) * area
        dH0dy = np.dot(dP0dy,minv) * area
        dH1dx = np.dot(dP1dx,minv) * area
        dH1dy = np.dot(dP1dy,minv) * area
        dH2dx = np.dot(dP2dx,minv) * area
        dH2dy = np.dot(dP2dy,minv) * area
        idx0 = element[i,0]
        idx1 = element[i,1]
        idx2 = element[i,2]
    ...
    for i in range(node_num):
        node_vel_flatten[i*2 + 0] = node_force[i,0]
        node_vel_flatten[i*2 + 1] = node_force[i,1]
    
    dx = np.dot(np.linalg.inv(kmat),node_vel_flatten) 
